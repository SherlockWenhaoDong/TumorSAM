...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False):
        ...
        self.model = build_sam3_image_model(
            ...
            bpe_path="/home/wenhao/workspace/TumorSAM3/checkpoints/sam3/bpe_simple_vocab_16e6.txt.gz",
            eval_mode=False
        )
        ...
        # ✅ Load existing LoRA weights (for finetuning)
        if 'early' in self.config["output"].get("output_dir", None):
            lora_ckpt = self.config["lora"].get("resume_from", None)

            if lora_ckpt is not None and os.path.exists(lora_ckpt):
                print_rank0(f"✅ Loading LoRA weights from {lora_ckpt}")
