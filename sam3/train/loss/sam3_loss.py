        ...
        with SAM3Output.iteration_mode(
            ...
            for stage_outputs, stage_targets in zip(find_stages, find_targets):
                ...
                for outputs, targets in zip(stage_outputs, stage_targets):
                    ...
                    if "pred_masks" in outputs and "masks" in targets:
                        pred = outputs["pred_masks"]  # [B, Q, H, W] or [B,1,H,W]
                        gt = targets["masks"]  # [B, H, W]
                        ...
                        def boundary_loss(pred, gt):
                            ...
                            return torch.mean(torch.abs(pred - gt_ds))

                        boundary = boundary_loss(pred_mask, gt)
                        cur_losses[CORE_LOSS_KEY] += 0.5 * boundary
